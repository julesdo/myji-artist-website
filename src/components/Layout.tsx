...
export function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 px-6 py-8 md:px-12 flex justify-between items-center bg-brand-cream/80 backdrop-blur-md border-b border-brand-charcoal/5 pointer-events-none">
      <Link 
        to="/" 
        className="pointer-events-auto font-serif text-2xl md:text-3xl tracking-[0.2em] uppercase hover:opacity-70 transition-opacity"
      >
        {brandName}
      </Link>
        ...
        {navLinks.map((link) => (
          <Link
            ...
            className={cn(
              ...
              location.pathname === link.path ? "text-brand-charcoal" : "text-brand-charcoal/40 hover:text-brand-charcoal"
            )}
          >
            {link.name}
            {location.pathname === link.path && (
              <motion.span 
                ...
                className="absolute -bottom-1 left-0 w-full h-px bg-brand-charcoal/40"
              />
            )}
          </Link>
        ))}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-brand-cream text-brand-charcoal z-[60] p-12 pointer-events-auto flex flex-col justify-between"
          >
            ...
            <div className="flex gap-6 border-t border-brand-charcoal/10 pt-8">
              {settings?.instagramUrl && (
                <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer">
                  <Instagram size={20} className="hover:text-brand-gold transition-colors cursor-pointer" />
                </a>
              )}
              {settings?.pinterestUrl && (
                <a href={settings.pinterestUrl} target="_blank" rel="noopener noreferrer">
                  <Link2 size={20} className="hover:text-brand-gold transition-colors cursor-pointer" />
                </a>
              )}
              {settings?.contactEmail && (
                <a href={`mailto:${settings.contactEmail}`}>
                  <Mail size={20} className="hover:text-brand-gold transition-colors cursor-pointer" />
                </a>
              )}
            </div>
          </motion.div>
